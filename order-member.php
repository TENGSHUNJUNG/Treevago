...
<div class="container">
    ...
    <div class="box-1">
        ...
        <div class="box-1-1">
            ...
            <img class="product-img" src="./images/CL/CL<?= $row['sid'] ?>.png" >
        </div>

        <div class="box-1-2">
            <h3>房型：<span><?= $row['roomname'] ?></span></h3>
            <br>
            <h4>人數：<span><?= $row['people'] ?></span></h4>
            <br>
             <h4>入住時間：<?= $_SESSION['room'][$row['sid']][0] ?></h4>
             <h4>退房時間：<?= $_SESSION['room'][$row['sid']][1] ?></h4>
            <br>
            <div class="room1_right_price"><h4>森林價：<span><?= $row['price'] ?></span> /天</h4></div>
        </div>

<!--        $_SESSION['buyer']['member_recipient_name']-->

        <div class="box-1-3">
            <div class="btn-control">
                <div class="btn_login delete"><a onclick="myFunction()">刪除</a></div>
            </div>
        </div>
    </div>
    <?php for($i=0; $i<=3; $i++):

        if(! empty($_SESSION['class']['time'][$i])and (! empty($_SESSION['class']['people'][$i])) ):
            $sid = $_SESSION['class']['sid'][$i];
    ...
    <div class="box-2">
        ...
        <div class="box-2-1">
            <img src="./images/CL/C<?= $c_data[$sid]['sid'] ?>.png" alt="">
        </div>
        <div class="box-2-2">
            <h3>課程名稱：<span><?= $c_data[$sid]['classname'] ?><span></h3>
            <br>
            <h4>人數：<span><?= $_SESSION['class']['people'][$i] ?></span></h4>
            <br>
            <h4>上課時段：<?= $_SESSION['class']['time'][$i] ?></h4>
            <br>
            <div class="room1_right_price"><h4>價錢：<span><?= $c_data[$sid]['price'] ?> </span>/人</h4></div>
        </div>
        <div class="box-2-3">
            <div class="btn-control">
                <div class="btn_login"><a href="treevago_class.php">重新選購</a></div>
            </div>
        </div>
    ...
    <div class="btn_back_box">
        <div class="btn_back"><a href="treevago_class.php">返回課程</a></div>
    </div>
    <div class="total"><h2>需預付訂金：</h2>&nbsp;<span><?= $row['deposit'] ?></span>&nbsp;&nbsp;<b>元</b></div>
    <?php endwhile; ?>
    <div class="care-box">
        <h2>【訂房須知】</h2>
...
<script>
    function myFunction() {
        if (confirm("刪除房型，會連同課程一併刪除\n"+"您確定要刪除嗎?")) {
            setTimeout(function() {
                location.href="./sess_destroy_order.php";
            },100);
        } else {
            return false;
        }
